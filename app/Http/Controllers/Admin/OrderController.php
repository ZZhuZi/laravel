...
class OrderController extends Controller
{
	...
	public function export(){
		...
		//导出的数据
		$exportData = [];
		$head = ['order_sn','goods_pricce','user_id','consignee','phone','shipping_name','pay_name']; //excel 的head头
		$exportData[] = ['订单号','用户总价','用户id','收货人','收货人手机','配送方式','支付方式'];
	...
	public function doImport(Request $request){
		...
		if($files->extension() != "xls" && $files->extension() !="xlsx"){
			return redirect()->back()->with('msg','文件格式不正确,请上传xls，xlsx后缀文件');
		}
